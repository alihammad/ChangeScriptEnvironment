...
--  select min(work_ord_completion_dt) from SOCOMM.f_bolton_add_churn
...
retl_store
FROM      IOVIEWS_SRD.oo_svcins svc   ---select * from  IOVIEWS_SRD.oo_svcins where svc_inst_id = '100008459580'
	INNER JOIN  IOVIEWS_SRD.oo_wrkord ord
	...
	INNER JOIN IOVIEWS_SRD.oo_svciex s -- select * from  IOVIEWS_SRD.oo_svciex where svc_inst_id = '100008459580'
 	...
 	INNER JOIN SOCOMM.bolton_rp_ref r
          ON cmp.component_id =  r.rate_plan_cd
          AND r.bolton_in = '1'
 	INNER JOIN IOVIEWS_SRD.oo_svcijn sj
          ON  sj.svc_inst_id = ord.svc_inst_id 
	 AND work_ord_completion_dt BETWEEN sj.open_dt-2 AND sj.close_dt
	LEFT OUTER JOIN  IOVIEWS_SRD.OO_WRKOJN   ordjn  --- select * from IOVIEWS_SRD.OO_WRKOJN where work_ord_no = 'QRD0967003'
...
select * from 
IOVIEWS_SRD.oo_btxnpa pkg
	INNER JOIN IOVIEWS_SRD.oo_btxncm cmp
          ON  cmp.int_pack_inst = pkg.int_pack_inst 
          left outer join IOVIEWS_SRD.AR_COMPONENT_DEFINITION_VALUES a
...
LEFT OUTER JOIN  IOVIEWS_SRD.ar_a100 a100
...
FROM vt_ga
INNER JOIN IOVIEWS_SRD.ar_s200 s200
...
FROM ga_subscr
LEFT OUTER JOIN IOVIEWS_SRD.ar_p200 p200 
...
INNER JOIN IOVIEWS_SRD.AR_PACKAGE_DEFINITION_REF pac
...
INNER JOIN
IOVIEWS_SRD.AR_COMPONENT_DEFINITION_VALUES ar
...
reps_id AS wrkjn_rep_id,
retl_store
FROM      IOVIEWS_SRD.oo_svcins svc
    INNER JOIN IOVIEWS_SRD.oo_wrkord ord
		ON  svc.svc_inst_id = ord.svc_inst_id
    ...
    INNER JOIN IOVIEWS_SRD.oo_btxnpa pkg
     ...
     INNER JOIN IOVIEWS_SRD.oo_btxncm cmp
          ON  cmp.int_pack_inst = pkg.int_pack_inst 
     INNER JOIN IOVIEWS_SRD.oo_svciex s   -- select * from IOVIEWS_SRD.oo_svciex where svc_inst_id = '100002407653'
          ON  s.svc_inst_id = ord.svc_inst_id
          AND s.ext_id_type IN ('20','601','15')     --601 DSLD; 15 - ULL
          AND s.start_dt <= ord.work_ord_completion_dt
          AND work_ord_completion_dt BETWEEN s.open_dt-2 AND s.close_dt
...
vt_churn_fin vt
	INNER JOIN
   		 IOVIEWS_SRD.oo_svcins svc   ---select * from  IOVIEWS_SRD.oo_svcins where svc_inst_id = '100008626232'
	...
	INNER JOIN IOVIEWS_SRD.oo_btxncm cmp
          ON  cmp.int_pack_inst = pkg.int_pack_inst 
LEFT OUTER  JOIN IOVIEWS_SRD.oo_svcijn sj
          ON  sj.svc_inst_id = ord.svc_inst_id 
...
DELETE FROM SOCOMM.f_bolton_add_churn 
...
FROM IOSHARE.OTT_AT_TRANSACTION   --- select * from IOSHARE.ott_at_transaction where service_id    ='sagrawal' = 'buzzabuzza'
...
FROM metv
LEFT OUTER JOIN 
IOSHARE.OTT_AT_TRANSACTION ott
...
UPDATE a
FROM metv_output a,
(SELECT acct_no, acct_name FROM IOVIEWS.acct 
...
,CURRENT_DATE AS load_date
,'adac_fbb' AS src_tbl
FROM SOCOMM.all_daily_add_churn ac
LEFT OUTER JOIN SOCOMM.cfg_sov_ref ref
	ON ac.contrct_nm = ref.rate_plan_ds
	AND ref.end_dt = '2899-12-31'
LEFT OUTER JOIN IOVIEWS_SRD.AR_COMPONENT_DEFINITION_VALUES com
...
FROM SOCOMM.all_daily_add_churn ac
INNER JOIN IOCFG.cfg_rate_plan_ref r
ON ac.bolton_rate_plan_cd = r.rate_plan_cd
INNER JOIN BOVIEWS_APP.L_SERVICE_EVENT se
ON ac.svc_stat_cd = service_event_cd
LEFT OUTER JOIN IOCFG.cfg_rate_plan_ref b 
...
FROM
IOVIEWS_SRD.OO_AWKORD wkr -- select * from IOVIEWS_SRD.OO_AWKORD where work_ord_no = 'UDQ3355001'

LEFT OUTER JOIN IOVIEWS_SRD.oo_btxnpa pkg
ON wkr.work_ord_no = pkg.work_ord_no
LEFT OUTER JOIN IOVIEWS_SRD.oo_btxncm com
ON pkg.int_pack_inst = com.int_pack_inst
LEFT OUTER JOIN
IOVIEWS_SRD.ar_component_definition_values ar
ON com.component_id = ar.component_id
AND ar.close_dt = '2899-12-31'
INNER JOIN  IOCFG.cfg_rate_plan_ref ref
...
SELECT acct_no  FROM SOCOMM.f_bolton_add_churn WHERE no_of_svc_x_acct IS NULL 
AND  work_ord_completion_dt BETWEEN (SEL start_dt FROM dt_param) AND (SEL end_dt FROM dt_param)         
AND src_tbl IN (
'Opom Extract',
'adac_fbb',
...
FROM IOVIEWS_SRD.oo_svcins inst
