...
--  
--  04-09-2013 : 
--  Change IPCFG.cfg_rate_plan_ref to spcomm.bolton_rp_ref as IPCFG is not maintain any more  
--  11-07-2014 : MC
...
SELECT     rate_plan_cd
                    ,rate_plan_ds
--FROM       IPCFG.cfg_rate_plan_ref 
--  04-09-2013
--  Change IPCFG.cfg_rate_plan_ref to spcomm.bolton_rp_ref as IPCFG is not maintain any more  
From spcomm.bolton_rp_ref
Where   bolton_in = '1'
...
SELECT rate_plan_cd, rate_plan_ds FROM spcomm.zz_cfg_rate_plan_ref
...
package_group
FROM ipviews_srd.AR_PACKAGE_DEFINITION_REF pk
INNER JOIN
ipviews_srd.AR_PACKAGE_COMPONENTS pk_com
ON pk.package_id = pk_com.package_id
AND pk_com.close_dt = '2899-12-31'
INNER JOIN
ipviews_srd.AR_COMPONENT_DEFINITION_VALUES com
...
,s200.inactive_date
 FROM 
ipviews_srd.ar_s100 s100
INNER JOIN
ipviews_srd.ar_s200 s200
...
UPDATE a
FROM basic_svc a,
(SELECT emf_config_id AS id, display_value FROM ipviews_srd.AR_EMF_CONFIG_ID_VALUES 
...
/*  select * from ipviews_srd.oo_wrkord ord
	INNER JOIN ipviews_srd.oo_btxnpa pkg
          ON  pkg.work_ord_no = ord.work_ord_no
	INNER JOIN ipviews_srd.oo_btxncm cmp
          ON  cmp.int_pack_inst = pkg.int_pack_inst 
...
acq_partnr_cmpny
FROM
 ipviews_srd.OO_WRKOJN   -- select * from ipviews_srd.oo_wrkojn where work_ord_no = 'PDBB292001'
...
DELETE FROM spcomm.f_bolton_base 
WHERE per_end_dt = (SELECT * FROM parameters);

.IF ERRORCODE <> 0 THEN .EXIT ERRORCODE


INSERT INTO spcomm.f_bolton_base
SELECT 
*
FROM 
output
...
COLLECT STATS ON spcomm.f_bolton_base COLUMN (svc_no);
COLLECT STATS ON spcomm.f_bolton_base COLUMN (bolton_rate_plan_cd);
COLLECT STATS ON spcomm.f_bolton_base COLUMN (per_end_dt);
COLLECT STATS ON spcomm.f_bolton_base COLUMN (basic_rate_plan_cd);

.IF ERRORCODE <> 0 THEN .EXIT ERRORCODE

-- TBC Mobile base -- 

DELETE FROM spcomm.f_tbc_base WHERE per_end_dt = ( ADD_MONTHS((CURRENT_DATE - EXTRACT(DAY FROM CURRENT_DATE)+1),0) -1) ;

.IF ERRORCODE <> 0 THEN .EXIT ERRORCODE

INSERT INTO  spcomm.f_tbc_base 
SELECT
 ADD_MONTHS((CURRENT_DATE - EXTRACT(DAY FROM CURRENT_DATE)+1),0) -1  AS per_end_dt,
 a.* FROM spcomm.f_tbc_acct_plans a
